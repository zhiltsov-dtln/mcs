...
def generate_new_oapi_token():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            logger.error('Incorrect OAPI SD login response: Status Code ' + str(r.status_code) + ' - ' + r.text)
...
def check_contragent_id(request, mail, retry=0):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            resp = r.json()
            logger.info(resp)
            logger.info("contragent is " + str(resp[0]['contragent']) + "; contragent_id is " + str(resp[0]['contragent_id']))
            ...
            logger.error('Incorrect OAPI SD issue response: Status Code ' + str(r.status_code) + ' - ' + r.text)
